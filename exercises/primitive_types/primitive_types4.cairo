...
fn sum_big_numbers(x: u8, y: u8) -> usize {
    let sum:felt252 = convert_to_felt(x) + convert_to_felt(y);
    sum.try_into().unwrap()
}
...
fn test_sum_big_numbers() {
    ...
    assert(sum_big_numbers(255_u8, 255_u8) == 510_usize, 'Something went wrong');
}
